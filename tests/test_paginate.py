import pytest
import re
from moffee.compositor import composite, Direction, Type, is_divider, get_header_level, rm_comments
...
def is_divider(line, type='-'):
    return re.match(r'^' + type * 3 + '$', line.strip()) is not None

def get_header_level(line):
    return len(re.match(r'^#+', line).group(0)) if re.match(r'^#+', line) else 0
